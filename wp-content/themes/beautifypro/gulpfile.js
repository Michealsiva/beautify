...
gulp.task( 'translate', function () {
   return gulp.src( './**/*.php')
     ...
     .pipe(wpPot( {
         domain        : 'beautify_pro',
         destFile      : 'beautifypro.pot',
         package       : 'BeautifyPro'
     } ))
    .pipe(gulp.dest('languages/beautifypro.pot'))
});       
...
gulp.task('clean', function() {
  return del(['images','js','languages/beautifypro.pot','rtl.css']);        
}); 


// Default task
gulp.task('default',['clean'] , function() {
  gulp.start('styles', 'scripts', 'images','translate', 'rtl');
});   
...
gulp.task('watch',function(){   
  browserSync.init({  
    files: ['./**/*.php'],    
    proxy: 'http://localhost/check/beautify/',
  });  
